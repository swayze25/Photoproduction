...
void jet_visualization (const vector<fastjet::PseudoJet> & , int);
double jet_shape_diff (const vector<fastjet::PseudoJet> &);
double jet_shape_incl (const vector<fastjet::PseudoJet> &);	

//Set the respective parameters :
double EtMin_C=17.0, R_C=1.0, crapmin=-1.0, crapmax=0.0, ycut_C = 0.0005, dcut_C;				
double EtMin_D=10.0, R_D=1.0, drapmin=-1.0, drapmax=2.0, ycut_D = 0.0005, dcut_D;				
double EtMin_R=10.0, R_R=1.0, rrapmin=-1.0, rrapmax=2.0, ycut_R = 0.0005, dcut_R;				
double EtMin_Q=10.0, R_Q=1.0, qrapmin=-1.0, qrapmax=2.5, ycut_Q = 0.0005, dcut_Q;
double EtMin_G=10.0, R_G=1.0, grapmin=-1.0, grapmax=2.5, ycut_G = 0.0005, dcut_G;
...
// Reading Quark initiated charged final state Electrons :
...
// Reading Gluon initiated charged final state Electrons :
...
int ev1=111,ev2=203,ev3=606; int c1=0,c2=0,c3=0;
...
int main(){
  ...
  cout << "		PseudoRapidity Range : " << grapmin << " to " << grapmax << endl;
	...
	double temp = 0;
	for (double ptr : rho_all) temp+=ptr;
	double s=rho_all.size();
	cout << "Differential Jet size for r(" << rr_diff << ") EtJetMin(" <<  EtMin_C << ") :: " << (temp/s) << endl;
	cout << s << endl;
	cout << c1 << " " << c2 << " " << c3;

	nqsubJets->Scale(1./nqsubJets->Integral(), "width");
	ngsubJets->Scale(1./ngsubJets->Integral(), "width");
