...
int main() {
	...
	pythia.readString("Beams:eA = 10.");				// electron : HERA=27.5GeV && EIC=18 GeV
	pythia.readString("Beams:eB = 100.");				// proton : HERA=820|920Gev && EIC=275 GeV
	...
	// Set Event Settings : EIC: 
	settings.mode("Photon:ProcessType", 0);												// Switch on : Automatic Mix
	pythia.readString("Photon:Q2max = 1.0");											// Maximal Q2
	pythia.readString("MultipartonInteractions:pT0Ref = 3");			// Use tuned pT0ref for photon-hadron (for LEP = 3)
	pythia.readString("PhaseSpace:pTHatMin = 10.");								// Limit partonic pThat.
