...
int main(int argc, char* argv[]) {
    ...
    for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
      ...
      for (int i = 0; i < pythia.event.size(); ++i){
        if ( pythia.event[i].isFinal() && pythia.event[i].isCharged() ) {
          ...
          //Checking the 'Status' of the final state Electron, PID = 11 
          if(pythia.event[i].id() == 2212) {
          	if(pythia.event[i].status() >= 11 && pythia.event[i].status() <= 19)	//beam particles
          	count[0]++;		
          	if(pythia.event[i].status() >= 21 && pythia.event[i].status() <= 29)	//particles of the hardest subprocess
          	count[1]++;
          	if(pythia.event[i].status() >= 31 && pythia.event[i].status() <= 39)	//particles of subsequent subprocesses
          	count[2]++;
          	if(pythia.event[i].status() >= 41 && pythia.event[i].status() <= 49)	//particles produced by initial-state-showers
          	count[3]++;
          	if(pythia.event[i].status() >= 51 && pythia.event[i].status() <= 59)	//particles produced by final-state-showers
          	count[4]++;
          	if(pythia.event[i].status() >= 61 && pythia.event[i].status() <= 69)	//particles produced by beam-remnant treatment
          	count[5]++;	
          	if(pythia.event[i].status() >= 71 && pythia.event[i].status() <= 79)	//partons in preparation of hadronization process
          	count[6]++;
			if(pythia.event[i].status() >= 81 && pythia.event[i].status() <= 89)	//primary hadrons produced by hadronization process
          	count[7]++;
          	if(pythia.event[i].status() >= 91 && pythia.event[i].status() <= 99)	//particles produced in decay process
          	count[8]++;
          	if(pythia.event[i].status() >= 101 && pythia.event[i].status() <= 109)	//long-lived (or stable) particles with heavy flavor
          	count[9]++;
          	if(pythia.event[i].status() >= 121)	//rest of the conditions
          	count[10]++;
  ...
  cout << "Number of Events : " << nEvent << endl;
	int x=0;
	cout << count[0] << "   " << count[1] << "   " << count[2] << "   " << count[3] << "   " << count[4] << "   " << count[5] << "   " << count[6] << "   " << count[7] << "   " << count[8] << "   " << count[9] << "   " << count[10] << "   " << endl;
	cout << "Total numbers of Electrons in the final state are :" << count[11] << endl;
